...
var disk_cache : bool
var badges_cached_folder : String

func _init(twitch_api : TwitchAPIConnection, disk_cache_enabled : bool = false, _badges_cached_folder := "user://badges") -> void:
	api = twitch_api
...
func get_badge(badge_id : String, channel_id : String = "_global", scale : String = "1x") -> Texture2D:
	...
	if ResourceLoader.has_cached(path):
		...
		var filepath : String = "%s/%s/%s_%s_%s.png" % [badges_cached_folder, channel_id, badge_data[0], badge_data[1], scale]
