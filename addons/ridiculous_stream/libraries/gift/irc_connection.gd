	...
	if (websocket != null && connection_state != ConnectionState.CONNECTION_FAILED && connection_state != ConnectionState.RECONNECTING):
		...
		match state:
			...
			WebSocketPeer.STATE_CLOSED:
				if (connection_state == ConnectionState.DISCONNECTED):
					print("Could not connect to Twitch IRC.")
					connection_state = ConnectionState.CONNECTION_FAILED
				elif(connection_state == ConnectionState.RECONNECTING):
					print("Reconnecting to Twitch IRC...")
					await(reconnect())
				else:
					connection_state = ConnectionState.DISCONNECTED
					print("Disconnected from Twitch IRC. [%s]: %s"%[websocket.get_close_code(), websocket.get_close_reason()])
...
func request_capabilities(caps : Array[String] = ["twitch.tv/commands", "twitch.tv/tags"]) -> void:
	last_caps = caps
...
func chat(message : String, channel : String = ""):
	...
	if(channel != ""):
		...
		if (last_state.has(channel)):
			var sender_data := SenderData.new(last_state[channel]["display-name"], channel, last_state[channels.keys()[0]])
...
func handle_message(message : String, tags : Dictionary) -> void:
	...
	match msg[1]:
		...
		"JOIN":
			var sender_data : SenderData = SenderData.new(user_regex.search(msg[0]).get_string(), msg[2], tags)
			user_joined.emit(sender_data)
		"PART":
			var sender_data : SenderData = SenderData.new(user_regex.search(msg[0]).get_string(), msg[2], tags)
			user_parted.emit(sender_data)
		_:
			unhandled_message.emit(message, tags)
